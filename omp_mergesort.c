...
#define SMALL    32  // Arrays size <= pequeños cambio a insertion sort
...
int main(int argc, char* argv[]) {
    ...
    int threads = atoi(argv[2]); // numero de threads consola
	 // Check  si se puede un paralelismo anidado
	  omp_set_nested(1);
    if (omp_get_nested() !=1 )
    {
...
// OpenMP merge sort  con un numbero de hilos
void mergesort_parallel_omp(float a[], int size, float temp[], int threads) {
    if ( threads == 1) {
//        printf("Thread %d begins serial merge sort\n", omp_get_thread_num());
     ...
     #pragma omp parallel sections num_threads(2)
       {
         // printf("Thread %d iniciar seccion recursiva\n", omp_get_thread_num());
  			#pragma omp section
  		        { //printf("Thread %d iniciar llamado recursivo\n", omp_get_thread_num());
  			mergesort_parallel_omp(a, size/2, temp, threads/2);}
  			#pragma omp section
  		        { //printf("Thread %d iniciar llamado recursivo\n", omp_get_thread_num());
  			mergesort_parallel_omp(a + size/2, size - size/2, temp + size/2, threads - threads/2);}
  			// el uso de  temp + size/2 es un cambio esencial de la version serial
      }
	   // la creacion de Thread ( allocation)  es imple dependiente
     // algunos threads  pueden ejefucutar  multiples secciones  mientras otros son (threads) secundarios
     // Merge de dos sub arrays atravez de temp
       merge(a, size, temp);
    } else {
...
void mergesort_serial(float a[], int size, float temp[]) {
    // cambio a insertion sort para arrays pequeños
     if (size < SMALL) {
    ...
    // lo de arriba parace no trabjar aprpiadamente en openmp ...
    //  hacer Merge de los dos sub arrays en un temp array
    merge(a, size, temp);
}
...
void merge(float a[], int size, float temp[]) {
    ...
    // copia del temp ordenado al arreglo principal
